# Code source: https://github.com/thiagoferreiraw/django-orm-optimization-talk/blob/main/shop/helpers.py
...
def _print_sql_statements(print_mode, time_elapsed_ms):
    for idx, query in enumerate(connection.queries, start=1):
        print(idx, _get_formatted_sql_statement(
            query["sql"], print_mode), "\n")
    total_sql_time_ms = sum(
        map(lambda q: float(q["time"]), connection.queries)) * 1000
    print(
